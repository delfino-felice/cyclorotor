...
difference() {
    ...
    diff("remove") {
        tube(h=H, ir=IR, wall=1.2, center=true) {
            attach(LEFT,RIGHT, align=BOT, overlap=1.2) cuboid([l, 17, h], center=true)
                attach(LEFT,RIGHT) cuboid([45,25,4])
                    attach(BOT,TOP,inside=true) cuboid([30,10,1.5], $tag="remove");
            attach(LEFT,RIGHT, align=BOT, overlap=1.2) cuboid([l+40, 3.8, 10], center=true);
        }
